...
void Button::SetRec(Rectangle rec, Color recColor = LIGHTGRAY) {
    this->useRatio = false;
    this->rec = rec;
    this->recColor = recColor;
}

void Button::SetRatio(float rx, float dx, float ry, float dy, float rw, float dw, float rh, float dh, Color recColor = LIGHTGRAY) {
    this->useRatio = true;
    this->rx = rx;
    this->dx = dx;
    this->ry = ry;
